...
const AiStockAgent = lazy(() =>
  import("@/components/ai-stock-agent").then((module) => ({
    default: module.AiStockAgent,
  })),
);
const StockPriceChart = lazy(() =>
  import("@/components/stock-price-chart").then((module) => ({
    default: module.StockPriceChart,
  })),
);
            ...
            <section className="panel-surface rounded-xl p-5 sm:p-6">
              <div className="flex flex-wrap items-start justify-between gap-6">
                ...
                <div className="text-right">
                  ...
                  <p
                    className={`tabular mt-1 flex items-center justify-end gap-1 text-sm ${toneClass}`}
                  >
                    {up ? (
              ...
              <dl className="mt-5 grid grid-cols-2 gap-4 border-t border-border pt-5 sm:grid-cols-3 lg:grid-cols-6">
                ...
                <Stat
                  label="Day range"
                  value={`${fmtNum(data.dayLow)} – ${fmtNum(data.dayHigh)}`}
                />
                <Stat
                  label="52-week range"
